    ...
    try {
        if ($action === 'delete') {
            ...
            if ($id > 0) {
                ...
                $stmt = $mysqli->prepare('INSERT INTO achievement_items (title, description, icon_class, image_url, sort_order) VALUES (?, ?, ?, ?, ?)');
                $stmt->bind_param('ssssi', $title, $description, $iconClass, $imagePath, $sortOrder);
                $stmt->execute();
                $stmt->close();
...
$achievements = [];
$result = $mysqli->query('SELECT * FROM achievement_items ORDER BY sort_order ASC, created_at DESC');
if ($result) {
